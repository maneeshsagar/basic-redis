...
func PutHandler(service service.Driver) gin.HandlerFunc {

	return func(c *gin.Context) {
		requestBody, err := io.ReadAll(c.Request.Body)
		if err != nil {
			fmt.Println("Error reading the request body")
			c.JSON(200, util.NewResponseWithModel(401, "Error reading the request body", nil))
			return
		}

		request := new(iolayer.PulRequest)

		err = json.Unmarshal(requestBody, request)
		if err != nil {
			fmt.Println("request body is not as expected")
			c.JSON(200, util.NewResponseWithModel(401, "request body is not as expected", nil))
			return
		}

		code, msg, err := service.PutService(request)
		if err != nil {
